...
GitHub Pull Request가 생성되면 Slack 채널에 자동으로 알림을 보내는 GitHub Action입니다.
...
```yaml
# email_mappings: GitHub username을 이메일로 매핑
email_mappings:
...
2. **이메일 매핑 확인**
   - `.github/pr-notify-config.yml` 파일의 이메일이 정확한지 확인
